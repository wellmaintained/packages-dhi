...
| Plone 4.3.20 + Zope 2.13.30 + SENAITE 1.3.5 | upstream pinned| upstream (no patches today)| `senaite-lims.vex.json`, `app-images.lock.yaml` + Py2-compatible transitive overlay (see [senaite-lims](senaite-lims/))                                              | grype against the SBOM (broader application-layer surface than later SENAITE lines) |
| Deployment composition                  | wellmaintained | wellmaintained engineers | `apps/senaite-1.3/deployments/`                                                                                                                                          | n/a — composition has no CVE surface of its own                  |
