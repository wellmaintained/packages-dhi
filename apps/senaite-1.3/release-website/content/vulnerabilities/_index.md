...
### Application-layer scale

A snapshot grype run surfaces roughly **371 findings** against the
1.3.x application layer (Plone 4.3 / Zope 2.13 / SENAITE 1.3.5 +
its transitive graph). The headline isn't "the 1.3.x line is
broken." It is the literal cost of supporting customers who haven't
taken the Plone-4-to-5 jump yet, paid by us in VEX-authoring hours
per release rather than by the operator in a forced upgrade.

Each finding is analysed against the heritage VEX policy below. As
the corpus fills in, the matrix's "WAIT" cells convert to specific
`not_affected` justifications or `affected` tracking issues.
...
to heritage application images. The relevant addenda for senaite-1.3:
...
extracted from each image's `vex.json` artifact. Until the
senaite-1.3 VEX authoring corpus is fully established, this table
renders mostly the "WAIT" / empty state; initial statements are
landing as authoring proceeds.
