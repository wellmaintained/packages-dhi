# SENAITE VEX Authoring Policy

This document is the operational handbook for authoring OpenVEX statements
against the SENAITE LIMS heritage image set
(`senaite-lims` and its `python-2.7` base layer).
...
> attacker-controlled patterns. SENAITE 2.0.0 does not accept regex
...
- **Plone 5.2.4 / Zope 4.5.5 stack.** ZODB-backed (no SQL
...
|---|---|---|
| `apps/senaite-1.3/VEX-POLICY.md` | This document. | Principal review when the policy changes (rare). |
| `apps/senaite-1.3/images/senaite-lims/senaite-lims.vex.json` | OpenVEX statements for the senaite-lims image. | Same commit as the image-level finding it triages. |
