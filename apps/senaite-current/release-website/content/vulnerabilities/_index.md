...
### Application-layer column: senaite-lims-current

The matrix carries one column for the application-layer image:
`senaite-lims` (the current line, on Plone 5.2.15 / Zope 4 with
SENAITE 2.6.0). The container list is data-derived from
`apps/senaite-current/app-images.yaml`, so it tracks the manifest
automatically.

The senaite-lims-current VEX corpus is currently a skeleton — initial
statements will be authored against the first grype scan of the
built image. Until that authoring pass lands, this table renders
mostly the "WAIT" / empty state; numbered-line snapshots
(`senaite-2.3`, `senaite-1.3`) carry their own corpuses in their
respective release-website distributions.
...
is data-derived from `apps/senaite-current/app-images.yaml`, so
columns appear for whichever images this release actually ships.
Until the senaite-lims-current VEX corpus is established, this table
renders mostly the "WAIT" / empty state; initial statements are
authored against the first grype scan of the built image.
