...
A snapshot grype run surfaces roughly **174 findings** against the
SENAITE 2.0.0 application layer (Plone 5.2.4 / Zope 4 + its
transitive graph). Each is analysed against the heritage VEX policy
below; as the corpus fills in, the matrix's "WAIT" cells convert to
specific `not_affected` justifications or `affected` tracking issues.
...
extracted from each image's `vex.json` artifact. Until the senaite
VEX authoring corpus is fully established, this table renders mostly
the "WAIT" / empty state; initial statements are landing as
authoring proceeds.
